...
public class GenMonthlyPaymentBlank extends Report {
	...
	public GenMonthlyPaymentBlank(ResultSet pr, String txt, String fullName) {
		this.pr = pr;
	...
	public void create(Info in, Image logo) throws Exception {
		...
		if (pr.next()) {
			...
			cost_code = job_num + "-" + pr.getString("division") + "-" + pr.getString("cost_code") + "-"
					+ pr.getString("phase_code");
			amount = pr.getDouble("amount");
		}

		Phrase p = new Phrase("Pay Request: " + company_name, DocHelper.font(8, Font.BOLD));
		...
		p = new Phrase("   Pay Request Info", DocHelper.font(
				12, Font.BOLD));
		c = new PdfPCell(p);
		...
		p = new Phrase("Print Name of Authorized Company Officer", DocHelper.font(8,
				Font.ITALIC));
		c = new PdfPCell(p);
		c.setBorder(0);
		DocHelper.center(c);
