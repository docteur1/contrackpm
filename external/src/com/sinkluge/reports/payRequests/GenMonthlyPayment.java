...
import com.sinkluge.util.BigString;


public class GenMonthlyPayment implements Report  {
	...
	public static String getQuery(String id) {
		return "select company_name, period, date_created, amount, pr.*, division, cost_code, phase_code, job_name, "
		+ "job_num from owner_pay_requests as opr join pay_requests as pr using (opr_id) join contracts "
		+ "using(contract_id) join job_cost_detail as jcd using(cost_code_id) join company using(company_id) join "
		+ "job on opr.job_id = job.job_id where pr.pr_id = " + id;
	}
	
	public static String getQuery2() {
		return "select txt from reports where id = 'prMonth'";
	}
	
	public GenMonthlyPayment(ResultSet pr, String txt, String fullName) throws SQLException {
		this.pr = pr;
	...
	public ByteArrayOutputStream create(Info in, Image logo) throws Exception {
		...
		String job_num = "0000", job_name = "ERROR";
		DecimalFormat df = new DecimalFormat("0000");
		double amount = 0;
		String period_out = "******";
		
		String inv_num = "Invoice Number";
		float co = 0;
		float vwctd = 0;
		float ptd = 0;
		float ret = 0;	
		...
		if (pr.next()) {
			...
			co = pr.getFloat("co");
			vwctd = pr.getFloat("adj_value_of_work");
		...
		p = new Phrase ("MONTHLY REQUEST FOR PAYMENT", DocHelper.font(16, Font.BOLD));
		c = new PdfPCell(p);
		...
		p = new Phrase(period_out, DocHelper.font(12));
		c = new PdfPCell(p);
		c.setBorder(0);
		t.addCell(c);
		...
		p = new Phrase("   Pay Request Details", DocHelper.font(12, Font.BOLD));
		c = new PdfPCell(p);
		...
		p = new Phrase("Adjusted Contract Amount", DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(Rectangle.LEFT);
		t.addCell(c);
		...
		p = new Phrase("Value of Work Completed to Date", DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(Rectangle.LEFT);
		t.addCell(c);
		...
		p = new Phrase(df.format(vwctd)	, DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(0);
		t.addCell(c);
		...
		p = new Phrase("SUBTOTAL (This Month's Billing)", DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(Rectangle.LEFT);
		t.addCell(c);
		...
		p = new Phrase(df.format(vwctd-ptd), DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(0);
		t.addCell(c);
		...
		p = new Phrase("Less This Month's Retention", DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(Rectangle.LEFT);
		t.addCell(c);
		...
		p = new Phrase("Amount Due This Pay Period", DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		...
		p = new Phrase(df.format(vwctd-ptd-ret), DocHelper.font(12));
		c = new PdfPCell(p);
		DocHelper.right(c);
		c.setBorder(Rectangle.BOTTOM);
		t.addCell(c);
		...
		if (pageNum != writer.getPageNumber()) document.add(image39);
		
		int orgPages = writer.getPageNumber();
		
		Attachments.addImageAttachments(pr_id, "PR", writer, document);
		
		document.close();

		return Attachments.addPDFAttachmentsAndStamp(pr_id, "PR", orgPages, baos.toByteArray());
