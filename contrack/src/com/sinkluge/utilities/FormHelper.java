...
public class FormHelper {
	...
	public static String curNoDec(float val) {
		return curNoDec((double) val);
	}
	
	public static String curNoDec(double val) {
		DecimalFormat df = new DecimalFormat("#,##0");
		return df.format(val);
	}
	
	public static String basicDecimal(double val) {
		return per(val);
	}
	
	public static String per(double val) {
		return (new DecimalFormat("#,##0.0")).format(val);
	}
	
	public static String timestamp (long l) {
		return timestamp(new Date(l));
	}
	...
	public static String left(String s, int l) {
		String ret = StringUtils.left(s, l);
		if (s != null && s.length() > l) ret += "...";
		return ret;
	}
