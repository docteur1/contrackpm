...
public class GenSubmittalToArch extends Report {
	...
	public ReportContact getReportContact(String id, Database db) throws Exception {
		String query = "select contact_id, company_id from submittals join job_contacts on architect_id = " +
			"job_contact_id where submittal_id = " + id;
		ResultSet rs = null;
	...
	public GenSubmittalToArch(String id, Database db, Attributes attr) throws Exception {
		this.id = id;
		type = Type.SUBMITTAL;
		...
		else subcontractor = attr.get("full_name");
		info.getStatement().close();
		query = "select company_name, name, contacts.address, contacts.city, contacts.state, user_id, submittal_type, "
